...
def test_summary_event_property_returns_most_recent() -> None:
    """Test that the summary_event property still returns the most recent summary.

    This maintains backward compatibility with existing code that uses this property.
    """
    messages = [message_event(f"Msg {i}") for i in range(3)]
    ...
    # The summary_event property should return the most recent one
    assert view.summary_event is not None
    assert view.summary_event.summary == "Recent summary"


def test_summary_event_index_property_returns_most_recent_offset() -> None:
    """Test that summary_event_index returns the index of the most recent summary.

    Maintains backward compatibility with code using this property.
    """
    messages = [message_event(f"Msg {i}") for i in range(3)]
    ...
    # Should return the offset from the most recent condensation
    assert view.summary_event_index == 2
