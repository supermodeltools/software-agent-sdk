...
# Model configurations dictionary - centralized model definitions
MODELS_CONFIG = {
...
def find_models_by_id(model_ids: list[str]) -> list[dict]:
    ...
    for model_id in model_ids:
        if model_id not in MODELS_CONFIG:
            available_ids = ", ".join(sorted(MODELS_CONFIG.keys()))
            error_exit(
                f"Model ID '{model_id}' not found. Available models: {available_ids}"
            )
        resolved.append(MODELS_CONFIG[model_id])
    return resolved
...
def main() -> None:
    ...
    if not model_ids:
        error_exit("No model IDs provided")
